...
double gauss_seidel_checkerboard(int n, double phi[n+1][n+1], int num_threads){ 
    ...
    int x, y;
    if (num_threads > n + 1) {
        (void)fprintf(stderr, "ERROR: Num_threads < n\n");
        exit(EXIT_FAILURE)
    }
    ...
    // Even update pass
    #pragma omp parallel for private(x, y) reduction(+:diff) schedule(static)
    for (x = 1; x < n; x++) {
        for (y = 1 + (x % 2); y < n; y += 2) {
...
int main()
{
    ...
    do {
        count++;
    } while (gauss_seidel_checkerboard(n,phi, 3) > 1.0e-10);
